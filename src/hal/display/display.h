...
public:
  ...
  inline uint8_t getPixelGray(int x, int y) const {
    ...
    // Rec. 601
    return (uint8_t) (((2990 * ((uint32_t) _buffer[index + 0])) + (5870 * ((uint32_t) _buffer[index + 1])) +
                       (1140 * ((uint32_t) _buffer[index + 2]))) /
                      10000);
...
public:
  virtual bool draw_image(const Image* image, DRAW_MODE draw_mode = DRAW_DEFAULT, int offset_x = 0, int offset_y = 0,
                          int rotate_cw = 0, uint8_t param = 0) {
    return draw_image_newcolor(
        image, [](int, int, uint8_t&, uint8_t&, uint8_t&, uint8_t, uint8_t) {}, param, draw_mode, offset_x, offset_y,
        rotate_cw);
  }
