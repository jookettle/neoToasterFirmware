...
bool EffectScript::initSequence(Display& display) {
  ...
  for (int i = 0; i < image_count; i++) {
    ...
    bool image_load = (image_usage[i] != 0 &&
                       (psramFound() || (image_usage_count < DYNAMIC_LOAD_COUNT || image_loaded < DYNAMIC_LOAD_COUNT)));
...
bool EffectScript::initVideo(Display& display) {
  ...
  if (ext != nullptr) {
    ...
    if (strcasecmp(ext, "png") == 0 || strcasecmp(ext, "jpg") == 0 || strcasecmp(ext, "jpeg") == 0) {
      ...
      auto asset = new Asset(filename.c_str(), _from_sd, Protogen.isRGB565(), _parser.getVideoInfo().loop,
                             _parser.getVideoInfo().fps);
      if (asset != nullptr) {
        if (!asset->isLoaded()) {
          delete asset;
        } else {
...
void EffectScript::processSequence(Display& display, const std::vector<DRAW_SEQUENCE>& sequence,
  ...
  if (current.method != Effect::SM_NONE) {
    ...
    if (_assets[current.image] == nullptr) {
      ...
      TF_LOGW(TAG, "processSequence: dynamic image loading, frame drops may occur (released: %d, loaded: %d)", released,
              loaded);
    }
  ...
  switch (current.method) {
    ...
    case Effect::SM_DRAW_270: {
      if (_assets[current.image]->getType() != Asset::ASSET_IMAGE) {
        TF_LOGE(TAG, "processSequence: asset type is not image. (asset: %d, type: %d)", current.image,
                _assets[current.image]->getType());
        _error = true;
        return;
      }
      ...
      if (current.color == Effect::PC_ORIGINAL) {
        display.draw_image(image, (DRAW_MODE) current.mode, current.offset_x, current.offset_y,
                           current.method - Effect::SM_DRAW);
      } else {
        display.draw_image_newcolor(image, _colorFunc, current.color, (DRAW_MODE) current.mode, current.offset_x,
                                    current.offset_y, current.method - Effect::SM_DRAW);
      }
    ...
    case Effect::SM_VIDEO_LOOP: {
      if (_assets[current.image]->getType() != Asset::ASSET_VIDEO) {
        TF_LOGE(TAG, "processSequence: asset type is not video. (asset: %d, type: %d)", current.image,
                _assets[current.image]->getType());
        _error = true;
        return;
      }
...
void EffectScript::processVideo(Display& display) {
  ...
  if (_video_legacy) {
    if (_step >= video_info.start && _step <= video_info.end) {
      if (_video_legacy_frame == nullptr) {
        if (video_info.path.length() > 255) {
          TF_LOGE(TAG, "processVideo: video path length exceededs 255 (%d, %s)", video_info.path.length(),
                  video_info.path.c_str());
          _error = true;
          return;
        }
      ...
      if (_video_legacy_frame->getType() != Asset::ASSET_IMAGE) {
        TF_LOGE(TAG, "processVideo: asset type is not image. (asset: %d, type: %d)", _step,
                _video_legacy_frame->getType());
        _error = true;
        return;
      }

      display.draw_image(_video_legacy_frame->getImage(), (DRAW_MODE) video_info.mode, _video_offset_x, _video_offset_y,
                         0);
    ...
    if (video_info.color == 0) {
      display.draw_image(asset->getVideo()->getImage(), (DRAW_MODE) video_info.mode, _video_offset_x, _video_offset_y,
                         0);
    } else {
      ...
      display.draw_image_newcolor(asset->getVideo()->getImage(), color_func, 0, (DRAW_MODE) video_info.mode,
                                  _video_offset_x, _video_offset_y, 0);
    }
...
size_t EffectScript::releaseSomeImages(Display& display, int current_sequence,
                                       const std::vector<DRAW_SEQUENCE>& sequence,
                                       const std::vector<DRAW_SEQUENCE>& other_sequence, size_t to_release) {
...
size_t EffectScript::loadNextImages(Display& display, int current_sequence, const std::vector<DRAW_SEQUENCE>& sequence,
                                    const std::vector<DRAW_SEQUENCE>& other_sequence, size_t to_load) {
  size_t load_count = 0;
