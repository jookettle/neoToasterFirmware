  ...
  switch (_step) {
    case 0:
      if (1) {
        ...
        oled.drawBitmap((OLED_WIDTH - BITMAP_DOWN_WIDTH) / 2, OLED_HEIGHT - BITMAP_DOWN_WIDTH, BITMAP_DOWN,
                        BITMAP_DOWN_WIDTH, BITMAP_DOWN_HEIGHT, 1);
...
void HUDBrightness::pressKey(uint16_t key, uint8_t mode) {
  if (key == 's' || mode == KM_KEYCODE && key == VK_DOWN) {
    ...
    restartTimer();
  } else if (key == 'f' || mode == KM_KEYCODE && key == VK_UP) {
    int brightness = getBrightness();
    brightness += 25;
    if (brightness > 100)
      brightness = 100;
