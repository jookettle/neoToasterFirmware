...
void HUDOptions::init() {
  ...
  addMenu("Boop", [](HUDBase* p, const char*) {
    ...
    hud_selector.selectString(list, Protogen._boopsensor.getEnabled() ? 1 : 0,
                              [](int index, void*) { Protogen._boopsensor.setEnabled(index == 0 ? false : true); });
    p->nextHUD(&hud_selector);
  });

  if (Protogen._rtc.isBegin()) {
    addMenu("Time", [](HUDBase* p, const char*) { p->nextHUD(&hud_timeset); });
  }
}
